...
<%
			...
			%>
<%Equipo equipo=controladorBD.dameEquipo(Integer.parseInt(cod_equipo)); 
Region region=controladorBD.dameRegion2(equipo.getCod_region()); 
Sede sede =controladorBD.dameSedeEquipo(equipo.getCod_equipo()); %>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	...
	<title>Leaguepedia | <%=equipo.getNombre() %></title>
</head>
<body>
					<!-- Logo -->
			...
			<!-- Contenido -->
			<div class="container">
				<h3 style="padding: 1.5% 1% 1% 1%"><%=equipo.getNombre() %></h3>
			</div>
			<hr style="margin: 0">
			<div class="container" style="padding: 2%;">
				<div class="card mb-3 center" style="border: 1px solid #a2a9b1;">
				  <div class="row no-gutters">
				    <div class="col-md-4">
				      <img src="images/teams/<%=equipo.getCod_equipo() %>.png" class="card-img" alt="<%=equipo.getNombre() %>" title="<%=equipo.getNombre() %>">
				    </div>
				    <div class="col-md-8" id="teamData" style="background-color: #f7f7f7;">
				      <div class="card-body">
				        <h5 class="card-title">Sede</h5>
				        <p class="card-text"><%=sede.getLocalizacion() %></p>
				        <hr>
				        <h5 class="card-title">País de fundación</h5>
				        <p class="card-text"><img src="images/countries/<%=equipo.getPais_fundacion() %>.png">&nbsp;<%=equipo.getPais_fundacion() %></p>
				        <hr>
				        <h5 class="card-title">Región</h5>
				        <div class="region-icon card-text"><%=region.getSiglas() %></div><%=region.getNombre() %><hr>
				        <h5 class="card-title">Propietario</h5>
				        <p class="card-text"><%=equipo.getPropietario() %></p>
				        <hr>
				        <h5 class="card-title">Patrocinadores</h5>
				        <p class="card-text">
				        <%for(int i=0; controladorBD.damePatrocinadoresDeEquipo(equipo.getCod_equipo()).size()>i; i++){ 
				        Patrocinador patrocinador = controladorBD.damePatrocinadoresDeEquipo(equipo.getCod_equipo()).get(i); %>
					        <%if(i==0){ %>
					        <%=patrocinador.getNombre() %>
					        <%}else{ %>
					        <br><%=patrocinador.getNombre() %>
					        <%} %>
				        <%} %>
				        </p>
				        <hr>
				        <h5 class="card-title">Fecha de fundación</h5>
				        <p class="card-text"><%=equipo.getFecha_fundacion() %></p>
...
<div class="container" style="padding: 2%;">
	<h4 style="text-align: center; padding-bottom: 2%; padding-top: 1%">Jugadores</h4>
	
		<%int j=0;
		for(int i=0; controladorBD.dameJugadoresEquipo(equipo.getCod_equipo()).size()>i; i++){ 
		Jugador jugador = controladorBD.dameJugadoresEquipo(equipo.getCod_equipo()).get(i);
		%>
			<%if(i%5==0 || i==0){ 
			j=0;%>
			<div class="card-group center" style="padding-bottom: 2%">
			<%}%>
				  <div id="card" class="card">
				    <img src="images/players/<%=jugador.getCod_jugador() %>.png" class="card-img-top" alt="<%=jugador.getNombre() %>" title="<%=jugador.getNombre() %>">
				    <div class="card-body" style="background-color: #f7f7f7;">
				      <h5 class="card-nick"><a href="Jugador.jsp?cod_jugador=<%=jugador.getCod_jugador()%>"><%=jugador.getApodo() %></a></h5>
				    </div>
				    <div class="card-footer">
				      <p style="margin: 0"><img src="
				      <%if(jugador.getRol().toLowerCase().contains("top")){ %>
				      images/rols/top.png
				      <%}else if(jugador.getRol().toLowerCase().contains("jungle")){ %>
				      images/rols/jungle.png
				      <%}else if(jugador.getRol().toLowerCase().contains("mid")){ %>
				      images/rols/mid.png
				      <%}else if(jugador.getRol().toLowerCase().contains("bot")){ %>
				      images/rols/bot.png
				      <%}else if(jugador.getRol().toLowerCase().contains("supp")){ %>
				      images/rols/supp.png
				      <%} %>" width="25px">&nbsp;<%=jugador.getRol() %></p>
				    </div>
				  </div>
			<%j=j+1;
			if(j==5 || i==((controladorBD.dameJugadoresEquipo(equipo.getCod_equipo()).size())-1)){ %>
			</div>
			<%} %>
		<%} %>
</div>
...
<hr width="90%" style="margin-top: 0; margin-bottom: 0">
<div class="container" style="padding: 2%;">
	<h4 style="text-align: center; padding-bottom: 2%; padding-top: 1%">Entrenadores</h4>
	
		<%j=0;
		boolean aligned=false;
		for(int i=0; controladorBD.dameEntrenadoresEquipo(equipo.getCod_equipo()).size()>i; i++){ 
		Entrenador entrenador = controladorBD.dameEntrenadoresEquipo(equipo.getCod_equipo()).get(i);
		int size = controladorBD.dameEntrenadoresEquipo(equipo.getCod_equipo()).size();
		%>
			<%if(i%5==0 || i==0){ 
			j=0;%>
			<div class="card-group center" style="padding-bottom: 2%">
			<%}%>
				<%if(size>1){ %>
					<%if(aligned==false){ %>
						<%if(size==2){ 
						aligned=true;%>
						<div class="card" style="max-width: 30%; background-color: #dee2e6;border: 0"></div>
						<%}else if(size==3){ 
						aligned=true;%>
						<div id="card" class="card" style="background-color: #dee2e6; border: 0"></div>
						<%}else if(size==4){ 
						aligned=true;%>
						<div class="card" style="max-width: 10%; background-color: #dee2e6; border: 0"></div>
						<%} %>
					<%} %>
				<div id="card" class="card">
				<%}else{ %>
				...
				<%} %>
				    <img src="images/coachs/<%=entrenador.getCod_entrenador() %>.png" class="card-img-top" alt="<%=entrenador.getNombre() %>" title="<%=entrenador.getNombre() %>">
				    <div class="card-body" style="background-color: #f7f7f7;">
				      <h5 class="card-nick"><a href="Entrenador.jsp?cod_entrenador=<%=entrenador.getCod_entrenador() %>"><%=entrenador.getApodo() %></a></h5>
				    </div>
				    <div class="card-footer">
				      <p style="margin: 0"><%=entrenador.getCategoria() %></p>
				    </div>
				  </div>
			<%j=j+1;
			if(j==5 || i==((controladorBD.dameEntrenadoresEquipo(equipo.getCod_equipo()).size())-1)){ %>
			</div>
			<%} %>
		<%} %>
</div>
				<%if(controladorBD.dameCampeonatosEquipo(equipo.getCod_equipo()).size()!=0){ %>
					<hr width="90%" style="margin-top: 0; margin-bottom: 0">
							<div class="table-wrapper">
								<table class="center thinner" style="margin-bottom: 3%; margin-top: 3%; border-collapse: collapse;">
									<thead>
										<tr>
											<th colspan="5"><h4 style="margin-bottom: 2%;">Campeonatos ganados</h4></th>
										</tr>
										<tr>
											<th></th>
											<th>Nombre</th>
									...
									<tbody>
									<%for(int i=0; controladorBD.dameCampeonatosEquipo(equipo.getCod_equipo()).size()>i; i++){
									Campeonato campeonato=controladorBD.dameCampeonatosEquipo(equipo.getCod_equipo()).get(i); %>
										<tr>
											...
											<td class="textCenter teamsImageContainer"><img class="teamsImage1" width="100%"; title="<%=controladorBD.dameEquipo(campeonato.getCod_equipo()).getNombre() %>";alt="<%=controladorBD.dameEquipo(campeonato.getCod_equipo()).getNombre() %>" src="images/teams/<%=campeonato.getCod_equipo()%>.png"></td>
										</tr>
									<%} %>
									</tbody>
								</table>

...
<%
			...
			String cod_campeonato="";
			
			//RECOGIDA DE VARIABLES
			if(!request.getParameter("cod_campeonato").equalsIgnoreCase("")){
				cod_campeonato=request.getParameter("cod_campeonato");
			}
			%>
<%Campeonato campeonato=controladorBD.dameCampeonato(Integer.parseInt(cod_campeonato)); %>
...
<head>
	...
	<title>Leaguepedia | <%=campeonato.getNombre() %></title>
</head>
<body>
			...
			<!-- Contenido -->
			<div class="container">
				<h3 style="padding: 1.5% 1% 1% 1%"><%=campeonato.getNombre() %></h3>
			</div>
			<hr style="margin: 0">
			...
			<table class="center thinner" style="margin-bottom: 3%; margin-top: 3%; border-collapse: collapse;">
									...
									<tbody>
										<tr>
											...
											<%} %>
											></td>
											<td class="textCenter responsiveText"><a href=""><%=campeonato.getNombre() %></a></td>
							...
							<div class="container">
								<h4 style="padding: 1.5% 1% 1.5% 1%; text-align: center;">Participantes</h4>
							</div>
							<div id="responsiveParticipants" class="container center">
							  <div style=" display: block;">
							  <%for(int i=0; controladorBD.dameEquiposCampeonato(campeonato.getCod_campeonato()).size()>i; i++){
								Equipo equipo = controladorBD.dameEquiposCampeonato(campeonato.getCod_campeonato()).get(i); %>
								<div class="Participants">
									<table class="center thinner" style="margin-bottom: 3%; margin-top: 3%; border-collapse: collapse; border: 1px solid #a2a9b1;background-color: #f7f7f7;">
										<tbody>
											<tr>
												<td id="participants"><a href="Equipo.jsp?cod_equipo=<%=equipo.getCod_equipo() %>"><img class="center" src="images/teams/<%=equipo.getCod_equipo() %>.png" alt="<%=equipo.getNombre() %>" title="<%=equipo.getNombre() %>" style="width: 80px;"></a></td>
											</tr>
										</tbody>
									</table>
								</div><%} %>

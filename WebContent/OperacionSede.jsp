...
<%			
	...
	String title="";
	if(tipo.equalsIgnoreCase("modificarsede")){
		title = "Modificar sede";
	}else if(tipo.equalsIgnoreCase("altasede")){
		title = "Alta sede";
	}else if(tipo.equalsIgnoreCase("bajasede")){
		title = "Baja sede";
	}
	%>
...
<body>
			...
			<!-- Contenido -->
			<div class="container">
				<h3 style="padding: 1.5% 1% 1% 1%"><%=title %></h3>
			</div>
			<hr style="margin: 0">
			
			<%if(tipo.equalsIgnoreCase("altasede")){ %>
			<article class="operaciones">
				<section class="col-12">
							<form action="operaciones.jsp?tipo=altasede" method="post">
								<div class="operaciones">
									<input type="text" name="localizacion" id="localizacion" placeholder="Localización de la sede" maxlength="50"/>
								</div>
								
								<div class="operaciones" style="padding-top: 2%;">
									<input type="submit" class="button alt" value="Dar de alta" />
								</div>
							</form>
				</section>
			</article>
			...
			<article class="operaciones">
				<section class="col-12">
							<form action="operaciones.jsp?tipo=bajasede" method="post">
								<div>
									<input type="submit" class="button alt" value="Dar de baja" />
								</div>
								  <label id='choose' for='options'>Seleccione la sede</label>
									<br clear='all' />
									<select id='cod_sede' name="cod_sede"  size="5" style='display:none;'>
									    <%for (int i=0; controladorBD.dameSedes().size()>i ; i++){
										Sede sede = controladorBD.dameSedes().get(i); %>
									    <option value="<%=sede.getCod_sede() %>"><%=sede.getLocalizacion() %></option>
								  		<%} %>
									</select>
							</form>
				</section>
			</article>
			<%}else if(tipo.equalsIgnoreCase("modificarsede")){ %>
			<article class="operaciones">
				<section class="col-12">
							<form action="operaciones.jsp?tipo=modificarsede" method="post">
								<div class="operaciones">
									<input type="text" name="localizacion" id="localizacion" placeholder="Localización de la sede" maxlength="50"/>
								</div>

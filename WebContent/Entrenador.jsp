...
<%
			...
			String cod_entrenador="";
			
			//RECOGIDA DE VARIABLES
			if(!request.getParameter("cod_entrenador").equalsIgnoreCase("")){
				cod_entrenador=request.getParameter("cod_entrenador");
			}
			%>
<%Entrenador entrenador=controladorBD.dameEntrenador(Integer.parseInt(cod_entrenador)); 
...
<head>
	...
	<title>Leaguepedia | <%=entrenador.getApodo() %></title>
</head>
<body>
			...
			<!-- Contenido -->
			<div class="container">
				<h3 style="padding: 1.5% 1% 1% 1%"><%=entrenador.getApodo() %></h3>
			</div>
			<hr style="margin: 0">
			<div class="table-wrapper">
				<table class="alt center"> 		
									<tbody>
										...
										<tr>
										  	<td class="textCenter" width='60' style="background-color:#b6cfe5; padding: 0 5px 0 5px">Entrenadores</td>
										  	<td width='300' style="padding: 5px 15px 5px 15px;">
										  		<ul class="options">
										  		<%for(int i=0; controladorBD.dameEntrenadoresEquipo(entrenador.getCod_equipo()).size()>i; i++ ){ 
										  		Entrenador entrenadorEquipo = controladorBD.dameEntrenadoresEquipo(entrenador.getCod_equipo()).get(i); %>
										  			<%if(controladorBD.dameEntrenadoresEquipo(entrenador.getCod_equipo()).size()!=0){ %>
											  			<%if(Integer.parseInt(cod_entrenador)!=entrenadorEquipo.getCod_entrenador()){ %>
											  			<li><a href="Entrenador.jsp?cod_entrenador=<%=entrenadorEquipo.getCod_entrenador()%>"><%=entrenadorEquipo.getApodo() %></a></li>
											  			<%}else{ %>
											  			<li><a><%=entrenadorEquipo.getApodo() %></a></li>
											  			<%} %>
											  		<%} %>
												<%} %>
										  		</ul>
			...
			<div class="container" style="padding: 2%;">
				<div class="card mb-3 center" style="border: 1px solid #a2a9b1;">
				  <div class="row no-gutters">
				    <div class="col-md-4">
				      <img src="images/coachs/<%=entrenador.getCod_entrenador() %>.png" class="card-img" alt="<%=entrenador.getNombre() %>" title="<%=entrenador.getNombre() %>">
				    </div>
				    <div class="col-md-8" id="teamData" style="background-color: #f7f7f7;">
				      <div class="card-body">
				        <h5 class="card-title">Nombre completo</h5>
				        <p class="card-text"><%=entrenador.getNombre() %></p>
				        <hr>
				        <h5 class="card-title">Apodo</h5>
				        <p class="card-text"><%=entrenador.getApodo() %></p>
				        <hr>
				        <h5 class="card-title">País de nacimiento</h5>
				        <p class="card-text"><img src="images/countries/<%=entrenador.getNacionalidad()%>.png">&nbsp;<%=entrenador.getNacionalidad() %></p>
				        <hr>
				        <h5 class="card-title">Categoría</h5>
				        <p class="card-text"><%=entrenador.getCategoria() %></p>
				        <hr>
				        <h5 class="card-title">Fecha de nacimiento</h5>
				        <p class="card-text"><%=entrenador.getFecha_nacimiento()%><br>(<%=controladorBD.dameEdadJugador(entrenador.getFecha_nacimiento(), entrenador.getCod_entrenador()) %> años)</p>
				        <hr>
				        <h5 class="card-title">Equipo</h5>
				        <p class="card-text"><a href="Equipo.jsp?cod_equipo=<%=entrenador.getCod_equipo() %>"><img src="images/teams/<%=entrenador.getCod_equipo() %>.png" width="25px"></a> &nbsp;<%=equipo.getNombre() %></p>

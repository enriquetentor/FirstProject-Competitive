...
<head>
	...
	<title>Leaguepedia | Entrenadores</title>
</head>
<body>
<%
			//VARIABLES
									...
									<tbody>
										...
										<tr>
										  	...
										  	<td width='300' style="padding: 5px 15px 5px 15px;">
										  		<ul class="options">
										  			<%if(!regionGiven.equalsIgnoreCase("")){%>
										  			<li><a href="Entrenadores.jsp?region">Todos</a></li>
										  			<%}else{ %>
										  			<li><a>Todos</a></li>
										  			<%} %>
										  		...
										  		%>
										  			<%if(controladorBD.dameEntrenadoresRegion(region.getNombre()).size()!=0){ %>
											  			<%if(!regionGiven.equalsIgnoreCase(region.getNombre())){%>
											  			<li><a href="Entrenadores.jsp?region=<%=region.getNombre()%>"><%=region.getNombre() %></a></li>
											  			<%}else{ %>
			...
			<table class="alt center thinner" style="margin-bottom: 5%;">
									...
									<%if(regionGiven.equalsIgnoreCase("")){%>
										<%for(int i=0; controladorBD.dameEntrenadores().size()>i; i++){
										Entrenador entrenador = controladorBD.dameEntrenadores().get(i);
										Equipo equipo = controladorBD.dameEquipo(entrenador.getCod_equipo());%>
										<tr>
											<td width='75' rowspan="2" style="text-align: center;"><a href="Entrenador.jsp?cod_entrenador=<%=entrenador.getCod_entrenador()%>"><img title="<%=entrenador.getNombre() %>"; 
											width=100%; alt="<%=entrenador.getNombre()%>" src="images/coachs/<%=entrenador.getCod_entrenador() %>.png"></a></td>
											<td width='200' colspan="2" style="padding-left: 15px;"><h6 class="responsiveText" style="margin: 0; padding: 2%"><%=entrenador.getNombre()%></h6></td>
											<td class="responsiveText" rowspan="2" width='60' style="text-align: center;"><%=entrenador.getApodo()%></td>
											<td class="responsiveText" rowspan="2" width='60' style="text-align: center;"><%=entrenador.getCategoria() %></td>
											<td class="responsiveText" rowspan="2" width='60' style="text-align: center;"><%=entrenador.getFecha_nacimiento() %></td>										
										</tr>
										<tr>
										  <td width='100' style="text-align: center;"><a href="Equipo.jsp?cod_equipo=<%=entrenador.getCod_equipo() %>"><img id="responsiveMiniImageList" class="teamImage-playersList" title="<%=equipo.getNombre() %>"; 
										  alt="<%=equipo.getNombre() %>" src="images/teams/<%=entrenador.getCod_equipo() %>.png"></a></td>
										  <td width='100' style="text-align: center;"><img id="responsiveMiniImageList" title="<%=entrenador.getNacionalidad() %>"; width=25px; 
										  alt="<%=entrenador.getNacionalidad() %>" src="images/countries/<%=entrenador.getNacionalidad() %>.png"></td>
										</tr>
										<%} %>
									<%}else{ %>
										<%for(int i=0; controladorBD.dameEntrenadoresRegion(regionGiven).size()>i; i++){
										Entrenador entrenador = controladorBD.dameEntrenadoresRegion(regionGiven).get(i);
										Equipo equipo = controladorBD.dameEquipo(entrenador.getCod_equipo());%>
										<tr>
											<td width='75' rowspan="2" style="text-align: center;"><a href="Entrenador.jsp?cod_entrenador=<%=entrenador.getCod_entrenador()%>"><img title="<%=entrenador.getNombre() %>"; 
											width=100%; alt="<%=entrenador.getNombre()%>" src="images/coachs/<%=entrenador.getCod_entrenador() %>.png"></a></td>

...
<head>
	...
	<title>Leaguepedia | Equipos</title>
</head>
<body>
<%
			//VARIABLES
									...
									<tbody>
										...
										<tr>
										  	...
										  	<td width='300' style="padding: 5px 15px 5px 15px;">
										  		<ul class="options">
										  			<%if(!regionGiven.equalsIgnoreCase("")){%>
										  			<li><a href="Equipos.jsp?region">Todos</a></li>
										  			<%}else{ %>
										  			<li><a>Todos</a></li>
										  			<%} %>
										  		...
										  		%>
										  			<%if(controladorBD.dameEquiposRegion(region.getNombre()).size()!=0){ %>
											  			<%if(!regionGiven.equalsIgnoreCase(region.getNombre())){%>
											  			<li><a href="Equipos.jsp?region=<%=region.getNombre()%>"><%=region.getNombre() %></a></li>
											  			<%}else{ %>
			...
			<table class="center thinner" style="margin-bottom: 5%; margin-top: 3%; border-collapse: collapse;">
									<thead>
										<tr>
											<th colspan="4"><h4 style="margin-bottom: 2%;">Visión general</h4></th>
										</tr>
										<tr>
											<th></th>
											<th>Nombre</th>
											<th class="teamstableThinner">País de fundación</th>
											<th class="teamstableThinner">Región</th>
										</tr>
									</thead>
									<tbody>
									<%if(regionGiven.equalsIgnoreCase("")){%>
										<%for(int i=0; controladorBD.dameEquipos().size()>i; i++){
										Equipo equipo = controladorBD.dameEquipos().get(i);
										Region region = controladorBD.dameRegion2(equipo.getCod_region()); %>
										<tr>
											<td class="textCenter teamsImageContainer"><img class="teamsImage2" width="100%"; title="<%=equipo.getNombre() %>"; 
											alt="<%=equipo.getNombre() %>" src="images/teams/<%=equipo.getCod_equipo() %>.png"></td>
											<td class="textCenter responsiveText"><a href="Equipo.jsp?cod_equipo=<%=equipo.getCod_equipo() %>"><%=equipo.getNombre() %></a></td>
											<td class="textCenter responsiveText teamstableThinner"><img title="<%=equipo.getPais_fundacion() %>"; 
											alt="<%=equipo.getPais_fundacion() %>" src="images/countries/<%=equipo.getPais_fundacion() %>.png">&nbsp;<%=equipo.getPais_fundacion() %></td>
											<td class="textCenter responsiveText teamstableThinner"><div class="region-icon"><%=region.getSiglas() %>
											</div>&nbsp;<%=region.getNombre() %></td>
										</tr>
										<%} %>
									<%}else{ %>
										<%for(int i=0; controladorBD.dameEquiposRegion(regionGiven).size()>i; i++){
											Equipo equipo = controladorBD.dameEquiposRegion(regionGiven).get(i);
											Region region = controladorBD.dameRegion2(equipo.getCod_region());%>
										<tr>
											<td class="textCenter teamsImageContainer"><img class="teamsImage2" width="100%"; title="<%=equipo.getNombre() %>"; 
											alt="<%=equipo.getNombre() %>" src="images/teams/<%=equipo.getCod_equipo() %>.png"></td>

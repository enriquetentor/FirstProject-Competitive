...
<%
			...
			if(!request.getParameter("cod_jugador").equalsIgnoreCase("")){
				cod_jugador=request.getParameter("cod_jugador");
			}
			%>
<%Jugador jugador=controladorBD.dameJugador(Integer.parseInt(cod_jugador)); 
...
<head>
	...
	<title>Leaguepedia | <%=jugador.getApodo() %></title>
</head>
<body>
			...
			<!-- Contenido -->
			<div class="container">
				<h3 style="padding: 1.5% 1% 1% 1%"><%=jugador.getApodo() %></h3>
			</div>
			<hr style="margin: 0">
			<div class="table-wrapper">
				<table class="alt center"> 		
									<tbody>
										<tr>
											<td class="textCenter" width='60' colspan="2" style="background-color:#b6cfe5;padding: 5px 0px 5px 0px"><%=equipo.getNombre() %></td>
										</tr>
										<tr>
										  	<td class="textCenter" width='60' style="background-color:#b6cfe5; padding: 0 5px 0 5px">Jugadores</td>
										  	<td width='300' style="padding: 5px 15px 5px 15px;">
										  		<ul class="options">
										  		<%for(int i=0; controladorBD.dameJugadoresEquipo(jugador.getCod_equipo()).size()>i; i++ ){ 
										  		Jugador jugadorEquipo = controladorBD.dameJugadoresEquipo(jugador.getCod_equipo()).get(i);%>
										  			<%if(controladorBD.dameJugadoresEquipo(jugador.getCod_equipo()).size()!=0){ %>
											  			<%if(Integer.parseInt(cod_jugador)!=jugadorEquipo.getCod_jugador()){ %>
											  			<li><a href="Jugador.jsp?cod_jugador=<%=jugadorEquipo.getCod_jugador() %>"><%=jugadorEquipo.getApodo() %></a></li>
											  			<%}else{ %>
											  			<li><a><%=jugadorEquipo.getApodo() %></a></li>
											  			<%} %>
											  		<%} %>
												<%} %>
										  		</ul>
			...
			<div class="container" style="padding: 2%;">
				<div class="card mb-3 center" style="border: 1px solid #a2a9b1;">
				  <div class="row no-gutters">
				    <div class="col-md-4">
				      <img src="images/players/<%=jugador.getCod_jugador() %>.png" class="card-img" alt="<%=jugador.getNombre() %>" title="<%=jugador.getNombre() %>">
				    </div>
				    <div class="col-md-8" id="teamData" style="background-color: #f7f7f7;">
				      <div class="card-body">
				        <h5 class="card-title">Nombre completo</h5>
				        <p class="card-text"><%=jugador.getNombre() %></p>
				        <hr>
				        <h5 class="card-title">Apodo</h5>
				        <p class="card-text"><%=jugador.getApodo() %></p>
				        <hr>
				        <h5 class="card-title">País de nacimiento</h5>
				        <p class="card-text"><img src="images/countries/<%=jugador.getNacionalidad()%>.png">&nbsp;<%=jugador.getNacionalidad() %></p>
				        <hr>
				        ...
				        <p class="card-text"><%=jugador.getFecha_nacimiento()%><br>(<%=controladorBD.dameEdadJugador(jugador.getFecha_nacimiento(), jugador.getCod_jugador()) %> años)</p>
				        <hr>
				        <h5 class="card-title">Equipo</h5>
				        <p class="card-text"><a href="Equipo.jsp?cod_equipo=<%=jugador.getCod_equipo() %>"><img src="images/teams/<%=jugador.getCod_equipo() %>.png" width="25px"></a> &nbsp;<%=equipo.getNombre() %></p>
